...
import { AdvancedPDFGenerator, type AdvancedPDFReportData } from './advancedPdfGenerator';

describe('AdvancedPDFGenerator', () => {
  it('should create an advanced PDF document', () => {
    const generator = new AdvancedPDFGenerator();
    expect(generator).toBeDefined();
  });

  it('should generate advanced PDF with sample data', async () => {
    const mockData: AdvancedPDFReportData = {
      location: {
        name: 'Test Location',
    ...
    const generator = new AdvancedPDFGenerator();
    const pdf = await generator.generateAdvancedSolarReport(mockData);
